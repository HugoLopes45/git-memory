...
import { RECORD_MAX_RETRIES, REF_ROOT, read, record } from "../packages/mneo/src/index.ts";

import { type TempRepo, makeTempRepo } from "./helpers/repo.ts";

const SDK_PATH = fileURLToPath(new URL("../packages/mneo/src/index.ts", import.meta.url));
...
describe("mneo concurrency", () => {
  ...
  test("non-retryable git error bubbles up immediately, not as 'contention exceeded'", () => {
    ...
    const notARepo = mkdtempSync(join(tmpdir(), "mneo-not-a-repo-"));
    try {
      let captured: Error | null = null;
      try {
