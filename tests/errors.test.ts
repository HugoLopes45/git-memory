// Error contract: every public SDK entry point throws a typed GitMemoryError
...
import { context } from "../packages/git-memory/src/context.ts";
import {
  GitMemoryError,
  InvalidInputError,
  NotFoundError,
...
import { findProjectDir, installHook } from "../packages/git-memory/src/init-hook.ts";
...
describe("error contract — read", () => {
  ...
  test("missing slug throws NotFoundError with code NOT_FOUND", () => {
    try {
      ...
      expect(e).toBeInstanceOf(GitMemoryError);
...
describe("error contract — context graceful degradation", () => {
  test("no repo → returns empty text instead of throwing", () => {
    ...
    try {
      ...
      process.chdir(cwd);
      if (prev !== undefined) process.env.GIT_MEMORY_REPO = prev;
      rmSync(stray, { recursive: true, force: true });
    }
  });
});
...
describe("error contract — branchToScope strict (Slice 3)", () => {
  ...
  test("auto-detected branch with underscore throws InvalidInputError naming the recovery path", () => {
    ...
    try {
      ...
      expect((e as Error).message).toMatch(/GIT_MEMORY_SCOPE/);
    }
  });
...
describe("error contract — findRepo via rev-parse (Slice 3, M1)", () => {
  test("non-repo dir with a stray HEAD file is rejected (no false-positive)", () => {
    ...
    const prev = process.env.GIT_MEMORY_REPO;
    try {
      process.env.GIT_MEMORY_REPO = stray;
      try {
        findRepo();
        throw new Error("should have thrown");
      } catch (e) {
      ...
      if (prev !== undefined) process.env.GIT_MEMORY_REPO = prev;
      // biome-ignore lint/performance/noDelete: process.env requires delete; assignment to undefined coerces to "undefined"
      else delete process.env.GIT_MEMORY_REPO;
      rmSync(stray, { recursive: true, force: true });
    }
  });
});
...
describe("error contract — base hierarchy", () => {
  test("every typed error is an instance of GitMemoryError and Error", () => {
    const fixture = makeTempRepo();
    try {
      const cases: GitMemoryError[] = [];
      try {
        read({ repo: fixture.repo, slug: "missing", scope: "main" });
      } catch (e) {
        cases.push(e as GitMemoryError);
      }
      try {
      ...
      for (const c of cases) {
        expect(c).toBeInstanceOf(GitMemoryError);
