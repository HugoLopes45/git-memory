...
describe("git-memory SDK", () => {
  ...
  describe("record", () => {
    ...
    test("by overrides commit author/committer name; email default unchanged", () => {
      ...
      expect(out).toBe("alice agent@git-memory alice agent@git-memory");
    });

    test("by omitted: falls back to GIT_MEMORY_AUTHOR, then 'git-memory'", () => {
