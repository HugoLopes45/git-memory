...
describe("mneo CLI — failure isolation", () => {
  ...
  test("MNEO_REPO=/nonexistent → exit 0, empty additionalContext", () => {
    const { code, stdout } = runCli(["context"], {
      cwd: tmpdir(),
      env: { MNEO_REPO: "/this/path/does/not/exist/anywhere" },
    });
    expect(code).toBe(0);
  ...
  describe("init-hook", () => {
    ...
    test("creates .claude/settings.json with SessionStart entry + matcher", () => {
      ...
      expect(entry.hooks[0].command).toBe("npx -y mneo context --budget 2000");
